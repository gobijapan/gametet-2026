...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!msnv || !name || !password || !confirmPassword) {
      ...
      // 1. Kiểm tra MSNV và Họ tên có khớp không
      ...
      if (employeeData.name.toUpperCase() !== name.toUpperCase()) {
        setError('Họ tên không khớp với mã số nhân viên!');
        setLoading(false);
        return;
      }
      ...
      // 2. Tạo email ảo và tạo user trong Firebase Authentication
      const syntheticEmail = `${msnv.toLowerCase()}@gametet.local`;
